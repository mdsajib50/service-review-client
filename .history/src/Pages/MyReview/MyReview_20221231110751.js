...
const MyReview = () => {
    ...
    return (
        <div className='mt-12 ml-16 grid gap-6 grid-cols-1 md:grid-cols-2  lg:grid-cols-3'>
            { 
                reviews ?
                reviews.map(review => <ReviewCard key={review._id} review={review} handelDelete={handelDelete}></ReviewCard>) : <h1 className='text-center ml-8'>No reviews were added</h1>
            }
        </div>
    );
};
